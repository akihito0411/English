...
def save(results, term , initials,name):
    #case_number = str(len(results)).zfill(3)--このコードはbananasが何行含まれているかを示している。
    case_number = name.zfill(3)
    current_time = datetime.now().strftime("%Y%m%d-%H%M")
    filename = f"{case_number}-{current_time}-{initials}-{term}.txt"
    with open(filename, 'w', encoding='utf-8') as file:
        for left, term, right in results:
            file.write(f"[{left}] \033[{term}]\033[{right}]\n")
